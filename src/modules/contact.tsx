'use client';

import { startTransition, useState } from 'react';
import Link from 'next/link';
import { RestrictToWindow } from '@dnd-kit/dom/modifiers';
import { DragDropProvider, useDraggable } from '@dnd-kit/react';
import { ChevronsDown, Github, Linkedin, Mail, X } from 'lucide-react';

import {
  Drawer,
  DrawerClose,
  DrawerContent,
  DrawerHeader,
  DrawerTitle,
} from '@/components/ui/drawer';
import { useKernel } from '@/components/hooks/use-kernel';
import { useIsMobile } from '@/components/hooks/use-mobile';
import { useSounds } from '@/components/hooks/use-sounds';

type Coordinates = {
  x: number;
  y: number;
};

const defaultCoordinates: Coordinates = {
  x: 800,
  y: 400,
};

export const ContactButton = () => {
  const { setIsContactOpen } = useKernel();

  const { play } = useSounds();

  const handleClick = () => {
    play({ id: 'click2' });
    setIsContactOpen(true);
  };

  return (
    <button
      className='glass-hover flex flex-1 items-center justify-center py-2 uppercase hover:cursor-pointer'
      onClick={handleClick}
    >
      Contact
    </button>
  );
};

export const ContactWindow = () => {
  const isMobile = useIsMobile();
  const [{ x, y }, setCoordinates] = useState<Coordinates>(defaultCoordinates);
  const { focusWindow } = useKernel();

  if (isMobile) {
    return <ContactDrawer />;
  }

  return (
    <DragDropProvider
      onDragStart={() => {
        focusWindow('contact');
      }}
      onDragEnd={({ operation }) => {
        startTransition(() => {
          setCoordinates(({ x, y }) => ({
            x: x + operation.transform.x,
            y: y + operation.transform.y,
          }));
          console.log('contact', x, y);
        });
      }}
    >
      <ContactDraggable
        top={y}
        left={x}
      />
    </DragDropProvider>
  );
};

const ContactDraggable = ({ top, left }: { top: number; left: number }) => {
  const { isContactOpen, setIsContactOpen, focusWindow, getWindowZIndex } = useKernel();

  const { ref, handleRef } = useDraggable({
    id: 'projects-draggable',
    modifiers: [RestrictToWindow],
  });

  const { play } = useSounds();

  if (!isContactOpen) return null;

  const style: React.CSSProperties = {
    position: 'absolute',
    top,
    left,
    zIndex: getWindowZIndex('contact'),
  };

  const handleClick = () => {
    focusWindow('contact');
  };

  const handleDragStart = () => {
    focusWindow('contact');
  };

  return (
    <article
      className='bg-background relative w-[368px] rounded-xs border font-sans'
      ref={ref}
      style={style}
      onClick={handleClick}
      onMouseDown={handleDragStart}
    >
      <div className='bg-primary text-primary-foreground flex items-center'>
        <div
          ref={handleRef}
          className='flex-1'
        >
          <p className='cursor-grab p-2 text-xs font-bold tracking-wide uppercase'>contact //</p>
        </div>
        <span
          onClick={() => {
            play({ id: 'click1' });
            setIsContactOpen(false);
          }}
          className='hover:bg-background/40 flex cursor-pointer items-center justify-center rounded-xs p-1'
        >
          [<X className='size-4' />]
        </span>
      </div>

      <div className='text-primary p-2'>
        <p className='text-sm'>Here are some ways to get in touch with me:</p>
        <div className='flex flex-col items-center gap-2 pt-2 text-sm'>
          <Link
            href='mailto:nathanm@hanascript.com'
            target='_blank'
            rel='noopener noreferrer'
            className='hover:border-secondary group w-full rounded-xs border p-2 hover:cursor-pointer'
          >
            <div className='group-hover:text-secondary flex items-center justify-center'>
              <span className='mr-2'>
                <Mail className='size-4' />
              </span>
              <span>HELLO@HANASCRIPT.COM</span>
            </div>
          </Link>
          <Link
            href='https://github.com/hanascript'
            target='_blank'
            rel='noopener noreferrer'
            className='hover:border-secondary group w-full rounded-xs border p-2 hover:cursor-pointer'
          >
            <div className='group-hover:text-secondary flex items-center justify-center'>
              <span className='mr-2'>
                <Github className='size-4' />
              </span>
              <span>GITHUB.COM/HANASCRIPT</span>
            </div>
          </Link>
          <Link
            href='https://www.linkedin.com/in/nathan-marcellous/'
            target='_blank'
            rel='noopener noreferrer'
            className='hover:border-secondary group w-full rounded-xs border p-2 hover:cursor-pointer'
          >
            <div className='group-hover:text-secondary flex items-center justify-center'>
              <span className='mr-2'>
                <Linkedin className='size-4' />
              </span>
              <span>LINKEDIN.COM/IN/NATHAN-MARCELLOUS/</span>
            </div>
          </Link>
        </div>
      </div>
    </article>
  );
};

const ContactDrawer = () => {
  const { isContactOpen, setIsContactOpen } = useKernel();
  return (
    <Drawer
      open={isContactOpen}
      onOpenChange={setIsContactOpen}
    >
      <DrawerContent className='font-sans'>
        <DrawerHeader className='bg-primary flex flex-row items-center justify-between rounded-t-lg p-2'>
          <DrawerTitle className='text-primary-foreground text-sm font-bold uppercase'>
            Contact //
          </DrawerTitle>
          <DrawerClose
            className='text-primary-foreground hover:cursor-pointer'
            onClick={() => setIsContactOpen(false)}
          >
            <ChevronsDown className='size-4' />
          </DrawerClose>
        </DrawerHeader>
        <div className='text-primary p-2'>
          <p className='text-sm'>Here are some ways to get in touch with me:</p>
          <div className='flex flex-col items-center gap-6 pt-2 text-sm'>
            <Link
              href='mailto:nathanm@hanascript.com'
              target='_blank'
              rel='noopener noreferrer'
              className='hover:border-secondary group w-full rounded-xs border p-2 hover:cursor-pointer'
            >
              <div className='group-hover:text-secondary flex items-center justify-center'>
                <span className='mr-2'>
                  <Mail className='size-4' />
                </span>
                <span>HELLO@HANASCRIPT.COM</span>
              </div>
            </Link>
            <Link
              href='https://github.com/hanascript'
              target='_blank'
              rel='noopener noreferrer'
              className='hover:border-secondary group w-full rounded-xs border p-2 hover:cursor-pointer'
            >
              <div className='group-hover:text-secondary flex items-center justify-center'>
                <span className='mr-2'>
                  <Github className='size-4' />
                </span>
                <span>GITHUB.COM/HANASCRIPT</span>
              </div>
            </Link>
            <Link
              href='https://www.linkedin.com/in/nathan-marcellous/'
              target='_blank'
              rel='noopener noreferrer'
              className='hover:border-secondary group w-full rounded-xs border p-2 hover:cursor-pointer'
            >
              <div className='group-hover:text-secondary flex items-center justify-center'>
                <span className='mr-2'>
                  <Linkedin className='size-4' />
                </span>
                <span>LINKEDIN.COM/IN/NATHAN-MARCELLOUS/</span>
              </div>
            </Link>
          </div>
        </div>
      </DrawerContent>
    </Drawer>
  );
};
